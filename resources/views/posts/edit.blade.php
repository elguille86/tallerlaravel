<x-app-layout>
    ...
    <form action="{{ route('posts.update' , $post->id ) }}" method="post">
        ...
        @method('PUT')
        <label for="">
            Titulo : <input type="text"  value="{{ $post->title }}" name="title" 
        ...
        <label for=""> Contenido : 
        <textarea name="content" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-150 resize-y" >{{ $post->content }}</textarea>
        </label>
        <br/><br/>

        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md transition duration-150 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Editar Post</button>
    </form>
</x-app-layout>